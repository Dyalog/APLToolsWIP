:Class X509Cert
    :field Public Instance LDRC          ⍝ Save a ref to DRC to be used from X509Cert
    ...
    ∇ certs←ReadCertUrls;certurls;list
      ...
      :AndIf 0<1⊃⍴2⊃certurls
          certs←{⎕NEW X509Cert(LDRC(4⊃⍵)('URL'(1⊃⍵))('URL'(2⊃⍵)))}¨↓2⊃certurls
      :Else
          certs←⍬
      :EndIf
    ∇
    ...
    ∇ certs←ReadCertFromStore storename;cs
      ...
      :If 0=1⊃cs
      :AndIf 0<⍴2⊃cs
          certs←⎕NEW¨(2⊃cs){X509Cert(LDRC ⍺ ⍵)}¨⊂'MSStore'storename
      :Else
          certs←⍬
      :EndIf
    ∇
    ...
    ∇ certs←ReadCertFromFile filename;c;base64;tie;size;cert;ixs;ix;d;pc;temp
      ...
      :If 0<⍴ixs
          :For ix :In ixs
              ...
              d←base64 Decode d
              certs,←⎕NEW X509Cert(LDRC d('DER'filename))
      ...
      :Else
          ...
          certs,←⎕NEW X509Cert(LDRC cert('DER'filename))
              ⍝c.Origin←'DER' filename
              ⍝certs,←c
      :EndIf
      ⎕NUNTIE tie
