...
⍝
    ...
    ∇ r←Render;opts;sel;dsp;rc;rtab
      :Access public
      Use'JQuery'
      Use'⍎/Scripts/chained/jquery.chained.min.js'
      Use'⍕/Scripts/chained/ChainedSelect.css'
      SetUse
    ...
    ⍝ downside: visual attributes set on the control (in MS) will have no effect!
      ...
      SetInputName
      id∆←id
      r←((id)('name="',id,'" value="',(1↓#.HtmlUtils.enlist(⍕¨Selected),¨','),'"')'type=hidden'Add _.input).Render
      →(0∊2↑⍴Options)/0
      ...
      js←((ScriptFollows #.Strings.subst'%maxdepth%'(⍕maxdepth))#.Strings.subst'%prefix%'id∆),'$(function() {'
            ...
            ⍝   }
            ⍝ }
      SelectedOnAllLevels←#.HtmlUtils.enlist(⊂Options[;3]){parents ⍵↑⍺}¨Selected
      rtab←(2,⍴uo←∪Options[;3])⍴⊂''
      :For level :In uo
          z←Options[;3]=level
          opts←z⌿Options
      ...
      :EndFor
      js,←'});'
      rtab←(⌽1,1<⍴∪#.HtmlUtils.enlist rtab[1;])⌿rtab
      :If ~Horizontal ⋄ rtab←⍉rtab ⋄ :EndIf
      r,←'<table class="ChainedSelectNoBorders">',(#.HtmlUtils.enlist(⊂'<tr>'),(⊂'<td>'),¨(rtab,¨⊂'</td>'),⊂'</tr>'),'</table>'
    ...
    ∇ opts←{id}AddClasses(opts maxdepth);z;j
      ...
      :For z :In {⍵/⍳⍴⍵}opts[;3]=⌊/opts[;3]
          ...
          :AndIf opts[z;3]<maxdepth  ⍝ and not at lowest level...
              op5←¯1↓#.HtmlUtils.enlist(⍕¨opts[z;3]↓⍳maxdepth),¨','  ⍝ remember that we need to hide these levels...!
          :EndIf
          opts[z;5]←⊂op5
