...
⍝ 2017 05 09 Adam: included in 16.0, upgrade to code standards
    ...
    :Section UTILS
        ...
        Split←{dlb¨⍵⊆⍨⍺≠⍵}                       ⍝ Split ⍵ on ⍺, and remove leading blanks from each segment
        ...
        WIN←⎕SE.SALTUtils.WIN                    ⍝ running under Windows

        ∇ ok←Compatible ⍝ ≥ 16.0
            ok←16≤⊃⊃(//)'.'⎕VFI 2⊃⎕SE.SALTUtils.APLV
        ∇

        Assert←{~⍵:'Requires Dyalog APL version 16.0 or higher' ⎕SIGNAL 16}
    ...
    :Section BUILD
        ...
        ∇ r←Build args;file;prod;path;lines;extn;name;exists;extension;i;cmd;params;values;names;_description;_id;_version;id;v;target;source;wild;options;z;tmp;types;start;_defaults;f;files;n
    ...
    :Section UCMD
        ...
        ∇ r←List
            ...
            r.Parse←'1S -production -clear[=]' '1S -filter= -setup= -teardown= -suite= -verbose -quiet -halt -trace -repeat='
            r/⍨←Compatible
        ∇
        ...
        ∇ r←level Help Cmd
            ...
            :Case 'DTest'
                ...
                :Else
                    ...
                    r,←⊂'    -verbose        display more status messages while runnning'
                :EndIf
            :EndSelect
            :If 0=level
                r,←⊂']??',Cmd,' ⍝ for more info'
